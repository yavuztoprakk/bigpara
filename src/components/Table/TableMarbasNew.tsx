...
import HeaderColumnMarbasNew from "./HeaderColumnMarbasNew";
import ColumnMarbasNew from "./ColumnMarbasNew";
import { useTheme } from "../../theme/ThemeContext";
import { useRef, useState } from "react";

export interface ColumnMarbasNew {
	title: string;
	render?: (row: any) => React.ReactElement;
	renderHeader?: (col: any) => React.ReactElement;
	valueKey: string;
	width: number;
	textAlign?: string;
	type?: string;
...
const Table: React.FC<Props> = ({
	...
	const scrollYValue = useRef(new RNAnimated.Value(0)).current;
		...
		<View onLayout={handleLayout} style={styles.container}>
			...
			<View style={[styles.fixedColumnHeader, { height: rowHeight }]}>
				{columns[0].renderHeader ? (
					...
					<HeaderColumnMarbasNew {...columns[0]} />
				)}
			</View>
				...
				<RNAnimated.View
					style={[
						...
						},
						{
							transform: [
								{
									translateY: scrollYValue.interpolate({
										inputRange: [0, data.length * rowHeight],
										outputRange: [0, -(data.length * rowHeight)],
										extrapolate: "clamp",
									}),
								},
							],
			...
			>
				...
				>
					{scrollColumns.map((column, i) => (
						<HeaderColumnMarbasNew key={i} {...column} />
					))}
				</Animated.View>
			</View>
		</View>
